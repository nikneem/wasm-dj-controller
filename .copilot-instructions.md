...
### 🎯 Priority 2: Low-Latency Control Response
...
```
src/
├── audio-engine/          # Rust/C++ WebAssembly module
│   ├── Cargo.toml
│   ├── src/
│   │   ├── lib.rs        # Main Wasm entry point
│   │   ├── decoder.rs    # MP3 decoding wrapper
│   │   ├── phase_vocoder.rs  # Time-stretching algorithm
│   │   ├── pitch_shift.rs    # Pitch-shifting algorithm
│   │   ├── bpm_detection.rs  # BPM and rhythm analysis
│   │   ├── key_detection.rs  # Pitch/key detection
│   │   └── filters.rs    # DSP filters and utilities
│   └── wasm-pack.toml
├── frontend/              # Angular application (using PrimeNG)
│   ├── src/
│   │   ├── app/
│   │   │   ├── components/
│   │   │   │   ├── deck/        # Deck (left/right) - using PrimeNG Card, Button
│   │   │   │   ├── crossfader/  # Crossfader control - using PrimeNG Slider
│   │   │   │   ├── deck-controls/ # Pitch/tempo sliders - using PrimeNG Slider, Knob
│   │   │   │   ├── waveform/    # Waveform display - custom Canvas with PrimeNG Panel
│   │   │   │   └── status-bar/  # BPM and key info - using PrimeNG Toolbar
│   │   │   ├── services/
│   │   │   │   ├── audio.service.ts    # AudioContext management
│   │   │   │   ├── audio-worklet.service.ts  # AudioWorklet control
│   │   │   │   ├── track-loader.service.ts   # File loading
│   │   │   │   └── ui-sync.service.ts # UI↔Audio sync
│   │   │   └── models/
│   │   │       ├── deck.model.ts
│   │   │       ├── track.model.ts
│   │   │       └── audio-state.model.ts
│   │   └── assets/
│   │       └── audio-worklet-processor.ts  # AudioWorklet script
│   ├── angular.json
│   ├── tsconfig.json
│   └── package.json
...
- Use `wasm-bindgen` for Rust ↔ JS interop
...
- **Pitch Adjustment**: Range ±2 semitones via pitch-shifting
